...
def filter_latest_year_snapshot(
    df: pd.DataFrame,
    date_col: str,
    year: int,
    cust_id_col: str,
    latest_per_customer: bool,
) -> pd.DataFrame:
    if date_col not in df.columns:
        raise RuntimeError(f"Missing date_col '{date_col}' in input parquet.")
    if cust_id_col not in df.columns:
        raise RuntimeError(f"Missing cust_id_col '{cust_id_col}' in input parquet.")

    d = df.copy()
    d[date_col] = pd.to_datetime(d[date_col], errors="coerce")

    # keep only target year
    d = d.loc[d[date_col].dt.year == int(year)].copy()
    if len(d) == 0:
        raise RuntimeError(f"No rows found for year={year} using date_col={date_col}.")

    if latest_per_customer:
        # latest row per customer within that year
        d = d.sort_values([cust_id_col, date_col])
        d = d.groupby(cust_id_col, as_index=False).tail(1).copy()
...
def _extract_state_dict(maybe_model_or_sd):
    """
    Accept either:
      - state_dict (dict-like)
      - nn.Module (full model object)
    Return: state_dict (dict-like)
    """
    # state_dict is dict-like (most common)
    if isinstance(maybe_model_or_sd, dict):
        return maybe_model_or_sd

    # full torch module saved in ckpt
    if isinstance(maybe_model_or_sd, torch.nn.Module):
        return maybe_model_or_sd.state_dict()

    raise TypeError(f"Unsupported ckpt model payload type: {type(maybe_model_or_sd)}")
...
def load_gate_awac(ckpt_path: Path, hidden: int, dropout: float) -> Tuple[nn.Module, List[str], np.ndarray, np.ndarray]:
    ...
    sdict = _extract_state_dict(ck["model"])
    sdict = _remap_state_dict_keys(sdict)
...
def _remap_state_dict_keys(sdict: dict) -> dict:
    """
    Fix common naming drifts between training vs inference.
    Example: backbone.net.*  -> backbone.*
    """
    # Case: ckpt has backbone.net.X but model expects backbone.X
    if any(k.startswith("backbone.net.") for k in sdict.keys()):
        remapped = {}
        for k, v in sdict.items():
            nk = k.replace("backbone.net.", "backbone.")
            remapped[nk] = v
        return remapped

    # (Optional) opposite case, if ever needed:
    # if any(k.startswith("backbone.") for k in sdict.keys()) and model expects backbone.net.
    # you can add reverse mapping similarly.
...
def main() -> None:
    ...
    ap.add_argument("--date_col", type=str, default="t_date",
                    help="Date column in trajectories parquet (e.g., t_date).")
    ap.add_argument("--year", type=int, default=2025,
                    help="Keep only rows from this calendar year.")
    ap.add_argument("--cust_id_col", type=str, default="cust_id",
                    help="Customer id column used to pick latest row per customer.")
    ap.add_argument("--latest_per_customer", action="store_true",
                    help="If set, pick latest row per customer within the chosen year.")


    # How to map mu01 -> pct_points:
    ap.add_argument("--mag_scale_pct", type=float, default=40.0,
                    help="Scale to convert percent points. Use 40")
    ...
    # ---- filter to latest 2025 snapshot(s) ----
    df = filter_latest_year_snapshot(
        df,
        date_col=args.date_col,
        year=args.year,
        cust_id_col=args.cust_id_col,
        latest_per_customer=args.latest_per_customer,
    )

    print(f"After year filter: rows={len(df)} | year={args.year} | latest_per_customer={args.latest_per_customer}")
    ...
    print(f"Wrote parquet to: {saved_to}")
