...
iql_gate.py
-----------
Baseline IQL for Gate (HOLD vs NONHOLD).
- Uses repo GateActorCritic (dict outputs: logit, q, v)
- Config-driven train_loop(conf)
- Option-A checkpoint: ckpt["model"] stores nn.Module for serving compatibility
"""
...
class GateIQLConfig:
    train_parquet: str
    val_parquet: str
    out_ckpt: str = "checkpoints/baselines/gate_iql.pt"
    ...
    # IQL knobs
    expectile_tau: float = 0.7       # common: 0.7..0.9
    awr_temp: float = 2.0            # advantage temperature
    adv_clip: float = 2.0            # clip raw advantage
    w_adv_clip: float = 0.7          # clip exponent input
...
def train_loop(conf: GateIQLConfig) -> Dict[str, Any]:
    set_seed(conf.seed)
    ...
    sampler = build_weighted_sampler(
        ds_tr,
        mix0=conf.mix_hold, mix1=conf.mix_nonhold,
        boost0=conf.boost_hold, boost1=conf.boost_nonhold
    )
        ...
        for ep in range(1, epochs + 1):
            ...
            for s_np, a_np, r_np, s1_np, done_np, w_np in tr_loader:
                ...
                if mode == "bce":
                    ...
                    # V expectile loss towards Q(s,a)
                    diff = (q_a.detach() - v)  # q - v
                    v_loss = expectile_loss(diff, tau=conf.expectile_tau)

                    # Policy: AWR-weighted BC using advantage (q - v)
            ...
            print(f"[IQL-GATE:{name}] Ep {ep:02d} train_pi={np.mean(train_pi):.4f} | VAL F1={val_best['f1']:.3f} thr={val_best['thr']:.3f}")
...
def run(conf: GateIQLConfig) -> Dict[str, Any]:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p = argparse.ArgumentParser("IQL Gate Baseline (HOLD vs NONHOLD)")
    p.add_argument("--train-parquet", required=True)
    p.add_argument("--val-parquet", required=True)
    p.add_argument("--out", default="checkpoints/baselines/gate_iql.pt")
